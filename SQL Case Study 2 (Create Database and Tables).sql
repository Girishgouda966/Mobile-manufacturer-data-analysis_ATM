...
select l.State,c.Customer_Name from FACT_TRANSACTIONS f inner join DIM_CUSTOMER c on c.IDCustomer = f.IDCustomer inner join DIM_LOCATION l on f.IDLocation =l.IDLocation
...
select t2.[Model_Name],manufacturer_name, avg(t3.TotalPrice) as avg_price
...
order by qty desc) x)
group by  t2.[Model_Name],Manufacturer_Name
order by  avg_price
...
where in (select r.Manufacturer_Name from (select top 5 Manufacturer_Name, COUNT(f.Quantity)[qty] from FACT_TRANSACTIONS f inner join DIM_MODEL m on m.IDModel = f.IDModel inner join DIM_MANUFACTURER r on r.IDManufacturer = m.IDManufacturer 
group by Manufacturer_Name
order by  qty desc) x)
group by Manufacturer_Name, Model_Name
order by AVG(totalprice) ,qty desc



select top 5 Manufacturer_Name, COUNT(f.Quantity)[qty] from FACT_TRANSACTIONS f inner join DIM_MODEL m on m.IDModel = f.IDModel inner join DIM_MANUFACTURER r on r.IDManufacturer = m.IDManufacturer 
group by Manufacturer_Name
order by  qty desc
...
select m.Model_Name from FACT_TRANSACTIONS f inner join DIM_MODEL m on m.IDModel = f.IDModel 
inner join DIM_DATE d on d.DATE = f.Date 
where  m.Model_Name in 
(
select model_name from 
(select top 5 m.Model_Name,count(f.Quantity)[qty] from FACT_TRANSACTIONS f inner join DIM_MODEL m on m.IDModel = f.IDModel 
inner join DIM_DATE d on d.DATE = f.Date 
where [YEAR] = (2008 )
group by m.Model_Name
order by count(f.Quantity) desc) as s1
intersect 
select model_name from 
(select top 5 m.Model_Name, (f.Quantity)[qty] from FACT_TRANSACTIONS f inner join DIM_MODEL m on m.IDModel = f.IDModel 
inner join DIM_DATE d on d.DATE = f.Date 
where [YEAR] = (2009 )
group by m.Model_Name
order by count(f.Quantity) desc) as s2
intersect 
select model_name from 
(select top 5 m.Model_Name,count(f.Quantity)[qty] from FACT_TRANSACTIONS f inner join DIM_MODEL m on m.IDModel = f.IDModel 
inner join DIM_DATE d on d.DATE = f.Date 
where [YEAR] = (2010 )
group by m.Model_Name
order by count(f.Quantity) desc) as s3)



select distinct Manufacturer_Name,YEAR  from DIM_MANUFACTURER r inner join DIM_MODEL m on r.IDManufacturer = m.IDManufacturer 
inner join FACT_TRANSACTIONS f on f.IDModel = f.
inner join DIM_DATE d on d.DATE = f.Date 
where YEAR in (2010) except 
select distinct Manufacturer_Name, year  from DIM_MANUFACTURER r inner join DIM_MODEL m on r.IDManufacturer = m.IDManufacturer 
...
(select [Customer_Name], [TotalPrice], avg([TotalPrice])[avg_spnd], avg([Quantity]) [avg_qty], year, 
lag(avg([TotalPrice]), 1,0) over(PARTITION by [Customer_Name] order by year)) as prev_spend from DIM_CUSTOMER c 
inner join FACT_TRANSACTIONS f on c.i = [dbo].[FACT_TRANSACTIONS].[IDCustomer]
group by [Customer_Name], year([Date]), [TotalPrice])A
order by avg_spnd desc

select top 100 c.Customer_Name, year, avg(TotalPrice)[Avg_spend],AVG(Quantity)[Avg_qty],sum(TotalPrice) from FACT_TRANSACTIONS f inner join DIM_CUSTOMER c on f.IDCustomer = c.IDCustomer
inner join DIM_DATE d on f.Date = d.DATE
group by Customer_Name, year 


select Customer_Name ,TotalPrice, AVG(TotalPrice)[avg_spend],AVG(Quantity)[avg_qty],[YEAR],LAG(AVG([totalprice],1,0) over(PARTITION by [Customer_Name] order by from DIM_CUSTOMER c inner join FACT_TRANSACTIONS f on c.IDCustomer = f.IDCustomer 
inner join DIM_DATE d on f.Date = d.DATE